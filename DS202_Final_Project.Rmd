...
```{r}
# Example: ANOVA for comparing Employment across different Education Levels
anova_result <- aov(Employed ~ Education_Bachelor's_degree + Education_High_school_graduate, data=merged_data)
...
lm_model <- lm(Employed ~ ., data = merged_data)
summary(lm_model)

# Decision Tree Model
library(rpart)
dt_model <- rpart(Employed ~ ., data = merged_data, method = "anova")
summary(dt_model)
...
# K-means clustering
set.seed(123)
km_model <- kmeans(merged_data[, c("Employed", "Education_Bachelor's_degree", "Age_25_to_34")], centers = 3)
merged_data$Cluster <- km_model$cluster

# PCA for cluster visualization
library(factoextra)
pca_res <- prcomp(merged_data[, c("Employed", "Education_Bachelor's_degree", "Age_25_to_34")], scale. = TRUE)
fviz_pca_biplot(pca_res, label = "none", col.ind = merged_data$Cluster, addEllipses = TRUE)
...
```{r}
# Employment Rate Map
plot_usmap(data = merged_data, values = "Employed", labels = TRUE) +
  scale_fill_continuous(name = "Employment Rate (%)", label = scales::percent_format()) +
  labs(title = "Employment Rate by State in 2021")
